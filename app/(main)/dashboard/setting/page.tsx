...
const ProfileModal = ({ onClose }: ProfileModalProps) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      ...
      onClick={onClose}>
      <div className="bg-white rounded-[16px] overflow-hidden shadow-2xl" style={{ width: '510px' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ width: '100%', height: '220px', borderRadius: '10px 10px 0 0', overflow: 'hidden' }}>
          <img src="https://images.unsplash.com/photo-1556742044-3c52d6e88c62?auto=format&fit=crop&q=80" alt="Banner"
            style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
        <div className="flex items-center px-6 pt-5 pb-4" style={{ gap: '20px', borderBottom: '1px solid #F3F4F6' }}>
          <div className="shrink-0">
            <img src="/purchase(2).png" alt="Profile"
          ...
          <div className="text-right shrink-0">
            ...
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, i) => <span key={i} style={{ color: '#FF8A65', fontSize: '14px' }}>★</span>)}
              <span style={{ fontFamily: 'Inter', fontSize: '12px', color: '#9CA3AF', marginLeft: '3px' }}>(10)</span>
            </div>
          </div>
        </div>
        <div className="px-6 pt-5 pb-3">
          <h4 style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: '16px', color: '#1F3A93', marginBottom: '14px' }}>Location</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', borderBottom: '1px solid #F3F4F6', paddingBottom: '16px' }}>
            {[{ label: 'Location :', value: 'Los angeles, CA' }, { label: 'Zip Code :', value: '.' }, { label: 'Adress :', value: '.' }].map(({ label, value }) => (
        ...
        <div className="px-6 pt-4 pb-6">
          <h4 style={{ fontFamily: 'Inter', fontWeight: 500, fontSize: '16px', color: '#1F3A93', marginBottom: '14px' }}>Social Profile</h4>
          <div className="flex items-center" style={{ gap: '15px' }}>
            {['WhatsApp', 'Facebook', 'X', 'Messenger', 'Email'].map(name => (
              <img key={name} src="" alt={name}
  ...
  return (
    <div className="flex bg-[#F9FAFB] min-h-screen" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="w-full px-8 pt-6 pb-10 flex flex-col">
          <h2 className="text-[24px] font-semibold text-[#1F3A93] mb-6">Setting</h2>

          {/* ✅ Banner: w-full instead of fixed 1019px */}
          <div className="relative bg-gray-200 overflow-hidden mb-6 w-full"
            style={{ height: '271px', borderRadius: '10px' }}>
            <img src="https://images.unsplash.com/photo-1556742044-3c52d6e88c62?auto=format&fit=crop&q=80"
          ...
          {/* ✅ Bottom container: w-full, no fixed 1015px */}
          <div className="flex gap-[16px] w-full" style={{ minHeight: '416px' }}>

            {/* Left Profile Card — fixed 330px as per design */}
            <div className="w-[330px] shrink-0 bg-white border border-gray-100 rounded-[10px] flex flex-col items-center pt-8 px-6">
              ...
              <p style={{ fontFamily: 'Inter', fontWeight: 400, fontSize: '10px', color: '#6B7280' }}>johndoe@gmail.com</p>
              <div className="mt-8 border-t border-gray-50 pt-6 flex flex-col gap-[30px] w-full">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => <span key={i} className="text-[#FF8A65] text-[14px]">★</span>)}
                    <span className="text-gray-400 text-[12px] ml-1">(10)</span>
                  </div>
                </div>
              </div>
              <div className="mt-auto mb-6 flex gap-3 w-full">
                <button className="flex-1 py-2 border border-[#1F3A93] text-[#1F3A93] rounded-md text-[13px] font-medium hover:bg-gray-50 transition-colors"
            ...
            {/* ✅ Right Form Card — flex-1 fills remaining space */}
            <div className="flex-1 min-w-0 bg-white border border-gray-100 rounded-[10px] p-8 overflow-y-auto">
